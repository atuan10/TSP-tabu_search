...
vector<int> tabuSearch(vector<int> &initialTour, double **distance, int maxIterations, int tabuSize, int maxNoImprovementPerMove)
{
    ...
    {
        cout << "Iteration " << iter + 1 << ": Current Best Cost = " << bestCost << " ,Move: " << currentMove << endl;
        stopiteration = iter;
        ...
        if (currentMove == 0)
        {
            tabuListTwoOpt.push_back(make_pair(twooptcity1, twooptcity2));
            if (tabuListTwoOpt.size() > tabuSize)
                tabuListTwoOpt.pop_front();
        }
        else if (currentMove == 1)
        {
            tabuListSwap.push_back(make_pair(swapcity1, swapcity2));
            if (tabuListSwap.size() > tabuSize)
                tabuListSwap.pop_front();
        }
        else if (currentMove == 2)
        {
            tabuListInsert.push_back(make_pair(citytoinsert, pos));
...
int main()
{
    ...
    string output = "outputcycling.txt";
