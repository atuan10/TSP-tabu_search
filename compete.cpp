                       ...
                       int maxIterations, int tabuSize, int maxNoImprovementPerMove)
{
    ...
    int twooptcity1 = -1, twooptcity2 = -1;
    int swapcity1 = -1, swapcity2 = -1;
    int citytoinsert = -1, pos = -1;
    for (int iter = 0; iter < maxIterations; ++iter)
    {
        ...
        for (int i = 0; i < n - 1; ++i)
        {
            for (int j = i + 1; j < n; ++j)
            {
                vector<int> neighbor = twoOptMove(curBestTour, i, j);
                double newCost = calculateCost(neighbor, distance);
        ...
        for (int i = 0; i < n - 1; ++i)
        {
            for (int j = i + 1; j < n; ++j)
            {
                vector<int> neighbor = swapCities(curBestTour, i, j);
                double newCost = calculateCost(neighbor, distance);
        ...
        for (int from = 0; from < n; ++from)
        {
            for (int to = 0; to < n; ++to)
            {
                ...
                vector<int> neighbor = insertCityMove(curBestTour, from, to);
                double newCost = calculateCost(neighbor, distance);

                if (find(tabuListInsert.begin(), tabuListInsert.end(), make_pair(from, to)) != tabuListInsert.end() && newCost >= bestCost)
                    continue;
        ...
        if (moveType == 0)
        { // Two-opt move
            tabuListTwoOpt.emplace_back(twooptcity1, twooptcity2);
            if (tabuListTwoOpt.size() > tabuSize)
                tabuListTwoOpt.pop_front();
        }
        else if (moveType == 1)
        {
            tabuListSwap.emplace_back(swapcity1, swapcity2);
            if (tabuListSwap.size() > tabuSize)
                tabuListSwap.pop_front();
        }
        else if (moveType == 2)
        {
            tabuListInsert.emplace_back(citytoinsert, pos);
            if (tabuListInsert.size() > tabuSize)
                tabuListInsert.pop_front();
        }
        if (countTostop > maxNoImprovementPerMove)
        {
            break;
        }
        cout << "Iteration " << iter + 1 << ": Current Best Cost = " << bestCost << ", Move: " << moveType << endl;
...
int main()
{
    ...
    string output = "comparetest2.txt";
    ...
    while (!file_name_list.empty())
    {
        ...
        op_sol = tabuSearch(op_sol, distance, n * 10, (int)sqrt(n * 10), 200);
